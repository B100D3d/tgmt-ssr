...
const Selector = ({ type, title, deletable }) => {
    ...
    const years = useMemo(
        () =>
            new Set(user.groups.map(({ year }) => year).sort((a, b) => a - b)),
        [user.groups]
    )
            ...
            ? useMemo(
                  () =>
                      user.groups.filter((group) =>
                          params.year ? group.year === +params.year : true
                      ),
                  [user.groups, params]
              )
            : type === "year"
    ...
    useEffect(() => {
        if (type === "subject") {
            ...
            if (user.role === "Teacher") {
                ...
                setSubjects(
                    user.subjects.filter((s) => subjectsId.includes(s.id))
                )
            }
        }
    }, [])
    ...
    return (
        <div className="user-main-container selector">
            ...
            {!deletable ? (
                ...
                <Link className="plus" to={`${location.pathname}/new`}>
                    +
                </Link>
            )}
...
const Item = ({ name, id, type, deletable }) => {
    ...
    return (
        <Link to={`${location.pathname}/${id}`}>
            ...
            >
                {deletable && (
                    <div className="delete" onClick={handleDelete} id={id}>
                        <span>&#215;</span>
                    </div>
                )}
